...
import { useClientOnly, useWindowSize, useScrollPosition } from '@/hooks/useClientOnly';
...
export default function UserNav() {
    ...
    // Use custom hooks to prevent hydration issues
    const isClient = useClientOnly();
    const { width: windowWidth } = useWindowSize();
    const scrollY = useScrollPosition();
    const scroll = scrollY > 50;
    ...
    const handleMouseEnter = (menu: string, submenu?: string) => {
      ...
      // Scroll to top on mount (only on client)
      useEffect(() => {
        if (isClient) {
          window.scrollTo(0, 0);
        }
    ...
    <>
        <div className={`header header-light head-shadow ${scroll ? 'header-fixed' : ''}`}>
            <div className="container">
                <nav id="navigation" className={isClient && windowWidth > 991 ? "navigation navigation-landscape" : "navigation navigation-portrait"}>
