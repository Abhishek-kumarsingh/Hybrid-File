                        ...
                        <div className="listing-img-wrapper">
                            <Link href={`/single-property-2/${item.id}`}>
                                {item.image.slice(0,1).map((el,index)=>{
                                    return(
                                        <Image src={el} width={600} height={400} sizes='(max-width: 768px) 100vw, 600px' priority={true} style={{width:'100%', height:'210px', objectFit: 'cover'}} className="img-fluid mx-auto rounded" alt={`Property image for ${item.name}`} key={index}/>
                                    )
                                })}
                            </Link>
                        </div>
